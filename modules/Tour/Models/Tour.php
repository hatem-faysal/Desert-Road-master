...
class Tour extends Bookable
{
    ...
    public function travelLocation()
    {
        return $this->hasMany(TravelLocation::class,'tour_id');
    }
    /**
    ...
    public function addToCart(Request $request)
    {
        ...
        $duration_in_day = max(1,ceil(($end_date->getTimestamp() - $start_date->getTimestamp()) / DAY_IN_SECONDS ) );
        $MulteRoom = MulteRoom::where('price',$request->multe_rooms_id)->first();
        $total =  (int)$MulteRoom->price??'' * $total_guests;
        ...
        $booking->multe_rooms_id = $MulteRoom->id??'';

...
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between mb20">
            <h1 class="title-bar">{{!empty($recovery) ? __('Recovery') : __("All Package")}}</h1>
            <div class="title-actions">
                @if(empty($recovery))
                <a href="{{route('tour.admin.create')}}" class="btn btn-primary">{{__("Add new package")}}</a>
                @endif
            </div>
        </div>
                            ...
                            @if($rows->total() > 0)
                                @foreach($rows as $row)
                                    <tr class="{{$row->status}}">
                                        ...
                                        @endphp
                                        @foreach ($countNew as $data)
                                            <span class="badge badge-publish">{{ $data->Location->name??'' }}</span>
                                            <br/>
                                        @endforeach
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            @if(empty($recovery))
                                                <a href="{{route('multilocation.admin.index',['id'=>$row->id])}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> {{__('Multi Location')}}</a>
                                            @endif
                                        </td>
                                        <td>
                                            @if(empty($recovery))
                                                <a href="{{route('multiroom.admin.index',['id'=>$row->id])}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> {{__('Multi Room')}}</a>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach

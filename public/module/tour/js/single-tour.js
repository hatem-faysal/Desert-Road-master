let date_start_new = document.getElementById('date_start_new').value;
let date_end_new = document.getElementById('date_end_new').value;
let priceNew = document.getElementById('priceNew').value;
let idTravel = document.getElementById('travel_id').value;
(function ($) {
    new Vue({
        ...
        computed:{
            ...
            total_price:function(){
                var me = this;
                    me.start_date=(date_start_new);
                    me.end_date=(date_end_new);
                if (me.start_date !== "") {
                    var total = 0;
                    var total_guests = 0;
                    var startDate = new Date(me.start_date).getTime();
        ...
        methods:{
                async AllDataTour() {
                    console.log(bookingCore.url);
                    const response = await fetch(bookingCore.url+"/tourCall/"+idTravel);      
                    this.AllData = await response.json();
              },
            handleTotalPrice: function () {
            },
            MulteRoomChange: function(e) {
            this.date_multe_room = e.target.value;
            },
            ...
            validate(){
                this.start_date=(date_start_new);
                this.end_date=(date_end_new);
                ...
                if(this.date_multe_room=='0')
                {
                    this.message.status = false;
                    this.message.content = 'Please select Type Room';
                    return false;
                }
                return true;
            },
            ...
            doSubmit:function (e) {
                ...
                $.ajax({
                    ...
                    data:{
                        service_id:this.id,
                         multe_rooms_id:this.date_multe_room,
                        service_type:'tour',
